                ...
                && x[1].entries.len() != 0
            {
                ...
                if is_lonely {
                    ...
                    if flags.end_sealed() {
                        ...
                        let entries =
                            VecSlice::new_extend(&mut final_unitigs_vec, linked.iter().copied());
                ...
                let (new_entry, oth_entry, vec_slice, mut flags) =
                    if flags.end_sealed() || (!flags.begin_sealed() && rand_bool.get_randbool()) {
                        (
                            first_entry,
                            last_entry,
                            VecSlice::new_extend(&mut current_unitigs_vec, entries.iter().copied()),
                            flags,
                        )
                    } else {
                        (
