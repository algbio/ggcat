...
pub struct MemoryTrackerManager {
    packet_sizes: DashMap<*const str, (usize, usize)>,
    executors_sizes: DashMap<*const str, (usize, usize)>,
}
    ...
    pub fn add_queue_packet<T: PacketTrait>(&self, packet: &T) {
        if packet.get_size() > 0 {
            let mut entry = self
                ...
                .or_insert((0, 0));
            entry.value_mut().0 += packet.get_size();
            entry.value_mut().1 += 1;
        }
    }
    ...
    pub fn print_debug(&self) {
        ...
        for executor in self.executors_sizes.iter() {
            println!(
                ...
                MemoryDataSize::from_bytes(executor.value().0),
                executor.value().1
            );
        }
        ...
        for packet in self.packet_sizes.iter() {
            println!(
                ...
                MemoryDataSize::from_bytes(packet.value().0),
                packet.value().1
            );
        }
    }
...
impl<E: Executor> MemoryTracker<E> {
    ...
    pub fn update_memory_usage(&mut self, usages: &[usize]) {
        ...
        let mut entry = self
            ...
            .get_mut(&(std::any::type_name::<E>() as *const str))
            .unwrap();
        entry.value_mut().0 -= self.last_memory_usage;
        entry.value_mut().0 += new_memory_usage;
