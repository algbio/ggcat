    ...
    for ParallelKmersMergeFinalExecutor<H, MH, CX>
{
    ...
    fn process_map(
        ...
        Self::get_kmers(global_data, map_struct, |hash, cread, rhentry| {
            ...
            //
            //     if out_seq.len() > k {
            //         let mut test_unitig_color =
            //             CX::ColorsMergeManagerType::<H, MH>::alloc_unitig_color_structure();
            //         CX::ColorsMergeManagerType::<H, MH>::join_structures::<false>(
            //             &mut test_unitig_color,
            //             &colors,
            //             &self.temp_color_buffer,
            //             1,
            //         );
            //
            //         let mut test_final_color_buffer =
            //             color_types::PartialUnitigsColorStructure::<H, MH, CX>::new_temp_buffer();
            //
            //         let test_colors =
            //             color_types::ColorsMergeManagerType::<H, MH, CX>::encode_part_unitigs_colors(
            //                 &mut test_unitig_color,
            //                 &mut test_final_color_buffer,
            //             );
            //
            //         CX::ColorsMergeManagerType::<H, MH>::debug_colors(
            //             &test_colors,
            //             &test_final_color_buffer,
            //             &out_seq[1..],
            //             &map_struct.rhash_map,
            //         );
            //     }
            ...
            //             );
            //
            //         let out_seq2 = out_seq
            //             .iter()
            //             .rev()
            //             .map(|x| {
            //                 Utils::decompress_base(Utils::conditional_rc_base(
            //                     Utils::compress_base(*x),
            //                     true,
            //                 ))
            //             })
            //             .collect::<Vec<u8>>();
