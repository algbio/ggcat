use crate::colors_memmap::ColorIndexType;
...
struct AsyncBuffer<T, P: AsyncSliceProcessor<TargetData = T>> {
    data: Option<UnsafeCell<Box<[T]>>>,
    position: AtomicUsize,
    reference: *const AsyncSliceQueue<T, P>,
}

impl<T, P: AsyncSliceProcessor<TargetData = T>> AsyncBuffer<T, P> {
    fn get_buffer(&self) -> &mut [T] {
        unsafe { (*(self.data.as_ref().unwrap().get())).deref_mut() }
    }
}

struct SliceReference<T, P: AsyncSliceProcessor<TargetData = T>> {
    buffer: Arc<AsyncBuffer<T, P>>,
    slice: Range<usize>,
}

impl<T, P: AsyncSliceProcessor<TargetData = T>> Drop for AsyncBuffer<T, P> {
    fn drop(&mut self) {
        unsafe {
            (&*self.reference)
                .available_buffers
...
pub struct AsyncSliceQueue<T, P: AsyncSliceProcessor<TargetData = T>> {
    chunks_buffers_pool: (
...
unsafe impl<T, P: AsyncSliceProcessor<TargetData = T>> Sync for AsyncSliceQueue<T, P> {}

impl<T: Copy, P: AsyncSliceProcessor<TargetData = T>> AsyncSliceQueue<T, P> {
    pub fn new(
        buffer_min_size: usize,
    ...
    fn alloc_buffer(&self, min_length: usize) -> Arc<AsyncBuffer<T, P>> {
        let buffer = self.available_buffers.pop().unwrap_or_else(|| unsafe {
            println!("Buffer allocated {}!", self.slices_queue.len());
