...
fn worker(context: &ExecutionContext, receiver: &Receiver<QueueData>) {
    ...
    let mut temp_buffers = vec![VEC; context.buckets_count];
    let mut temp_indexes = vec![VEC_USZ; context.buckets_count];
    ...
    let mut flush_buffer =
        |temp_indexes: &mut [Vec<usize>], temp_buffers: &mut [Vec<u8>], bucket: usize| {
            ...
            context.buckets.flush(bucket, |reader| {
        ...
        for x in data.iter_sequences() {
            ...
            while let Some(seq) = get_sequence() {
                ...
                let mut add_buffer = |index: usize, bucket: usize| {
                    temp_indexes[bucket].push(temp_buffers[bucket].len());
                    temp_buffers[bucket].extend_from_slice(&seq[last_index..(index + context.k)]);
                    // assert_reads(
    ...
    for i in 0..context.buckets_count {
