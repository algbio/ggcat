...
tokio::task_local!(pub static FLAME_LIBRARY: RefCell<FlameLibrary>);

#[derive(Debug)]
pub struct FlameLibrary {
    name: Option<String>,
...
fn commit_impl(library: &mut FlameLibrary) {
    use std::mem;
...
pub fn commit_thread() {
    FLAME_LIBRARY.with(|library| commit_impl(&mut *library.borrow_mut()));
}

impl Drop for FlameLibrary {
    fn drop(&mut self) {
        if ::std::thread::panicking() {
            return;
        }
...
fn end_impl<S: Into<StrCow>>(name: S, collapse: bool) -> u64 {
    ...
    let name = name.into();
    let delta = FLAME_LIBRARY.with(|library| {
        let mut library = library.borrow_mut();
        let epoch = library.epoch;
        let collector = &mut library.current;
